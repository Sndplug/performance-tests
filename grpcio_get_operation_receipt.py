...
"""Пополнение счёта"""
make_top_up_operations_request = MakeTopUpOperationRequest(
    status=OperationStatus.OPERATION_STATUS_COMPLETED,
    amount=fake.amount(),
    card_id=open_debit_card_account_response.account.cards[0].id,
    account_id=open_debit_card_account_response.account.id
)
make_top_up_operations_response: MakeTopUpOperationResponse = (
    operations_gateway_service.MakeTopUpOperation(make_top_up_operations_request)
)
print("Make top up operations response:", make_top_up_operations_response)


"""Получение чека по операции"""
get_operations_request = GetOperationReceiptRequest(operation_id=make_top_up_operations_response.operation.id)
get_operations_response: GetOperationReceiptResponse = operations_gateway_service.GetOperationReceipt(get_operations_request)
print("Get operations response:", get_operations_response)